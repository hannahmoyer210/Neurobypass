...
neurobypass <- read_csv("cleancodeR1.csv")
...
OverallApprovedsubset <- subset(neurobypass, approva_status == "preapproval")
...
ProcApprovedsubset <- subset(Procsubset, approva_status == "preapproval")
...
BroadBypassApprovedsubset <- subset(BroadBypass, approva_status == "preapproval")
...
industryfisher <- prop.test((x = c(ProcIndustrynumber,BroadBypassIndustrynumber)), (n = c(Procnumber,BroadBypassnumber)), p = NULL, alternative ="two.sided",correct = TRUE)

approvedfisher <-  prop.test((x = c(ProcApprovednumber,BroadBypassApprovednumber)), (n = c(Procnumber,BroadBypassnumber)), p = NULL, alternative ="two.sided",correct = TRUE)

Overalltermpvalue <- Overalltermfisher$p.value
ADtermpvalue <- ADtermfisher$p.value

industrypvalue <- Overalltermfisher$p.value
approvalpvalue <- ADtermfisher$p.value
...
SMDsubset <- SMDsubset %>%
  mutate(SMD_SE = case_when((variance_type=="95" ~ (abs((upperCI)-(lowerCI)))/(abs(qnorm((1-0.95)/2))*2)),
                  (variance_type=="97" ~ (abs((upperCI)-(lowerCI)))/(abs(qnorm((1-0.97)/2))*2)),
                  (variance_type=="0" ~  abs((Mean_difference)/qnorm(p_value/2)))))
...
labels2 <- c("PD","AD")
labels3 <- c("All Indications","AD", "PD","ALS", "Headache")

smdnb <- metabind(ADSMDpooling,PDSMDpooling, name = labels2)
waenb <- metabind(TotalwdAE,ADwdAEpooling,PDwdAEpooling, ALSwdAEpooling, HeadachewdAEpooling, name = labels3)
...
#PROCEEDED
#proceededpos
Overallpreceednew <- subset(Overallhasresults, Truebypass == "0")
...
##need to use full sample make full sample denominators
#preceded term
Overallpreceedfullnew <- subset(neurobypass, Truebypass == "0")
...
Overallbypasstermpercentnew <- (divide_by(Overallbypasstermnumbernew,Overallbypasscountfull))*100



Indicationvector3<-c("Pos Clinical P2 ","Any P2")

PositivityNnew<- c(Overallposcount,Overallposcount)

PositivityRatenew<- c(Overallpospercent, Overallpospercent)

proceedposNnew <- c(Overallprecposnumber, Overallprecposnumbernew)

proceedthatwerepospercentnew <- c(Overallprecedpospercent, Overallprecedpospercentnew)

bypassedposNnew<- c(Overallbypassposnumber,Overallbypassposnumbernew)

bypassedthatwerepospercentnew<- c(Overallbypasspospercent,Overallbypasspospercentnew) 
bypassedthatweretermpercentnew
TerminationNnew<- c(Overalltermcount, Overalltermcount)

TerminationRatenew<-c(Overalltermpercent,Overalltermpercent) 

proceedtermNnew <- c(Overallprectermnumber, Overallprectermnumbernew)

proceedthatweretermpercentnew <- c(Overallprecedtermpercent, Overallprecedtermpercentnew) 

bypassedtermNnew<- c(Overallbypasstermnumber,Overallbypasstermnumbernew)

bypassedthatweretermpercentnew<- c(Overallbypasstermpercent,Overallbypasstermpercentnew)
...
positivityandtermboth <- data.frame(Indication=rep(Indicationvector3),
                 Positivity_N=rep(PositivityNnew),                 
                 Positivity_Rate=rep(PositivityRatenew),
                 PrecPositivity_N =rep(proceedposNnew),
                 Positivity_Rate_when_proceeded_by_P2=rep(proceedthatwerepospercentnew),
                 BypassPositivity_N =rep(bypassedposNnew),
                 Positivity_Rate_when_bypassed=rep(bypassedthatwerepospercentnew),
                 Pvalues_forpos=rep(pvaluespos),
                 Termination_N=rep(TerminationNnew), 
                 Termination_Rate=rep(TerminationRatenew),
                 PrecTermination_N=rep(proceedtermNnew),
                 Termination_Rate_when_proceeded=rep(proceedthatweretermpercentnew),
                 BypassTermination_N=rep(bypassedtermNnew),
                 Termination_Rate_when_bypassed=rep(bypassedthatweretermpercentnew),
                 Pvalues_forterm=rep(pvaluesterm))


write.table(positivityandtermboth)
